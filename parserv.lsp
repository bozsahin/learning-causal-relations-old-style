...
; File name               :    parserv.lsp
;
; Created at              :    2/28/1990 00:06:47 by CL(NX) Indexer
;
; Indexed (source) file   :    parserv.lsp
;
;
...
(defun process_text (text)
  ...
  (mapcar #'(lambda (sentence)
              ...
              (princ "input is ") 
              (print sentence) 
              (let ((cd (process_sentence sentence)))
                (terpri)
                (princ "cd form is ")
                (print cd)))
          text))
...
(defun load_def ()
  (let ((packet (get *word* 'definition)))
    (cond (packet (add_stack packet)) (t (princ " --not in dictionary")))))
...
(proclaim '(special *part_of_speech* *cd_form* *subject* *predicates* *concept*
            ...
            *discourse_she* *discourse_it* *discourse_they* *time* *temprel*
            *co_agent* *discourse_co_agent*))
